...
const MangaPagesDiv = ({episodeImagesFilePath,manga,tektek,imgSrc,episodeNumber,mangaName,pageNumber}:any) => {
    ...
    return(
        ...
        <div className='group absolute top-[0] left-[0px] z-10 w-[80px] h-[100%] bg-transparent text-[#22E5DE]'>
            {tektek && 
            <Link href={`/manga/${mangaName}/${episodeNumber}/${pageNumber == 1 ? pageNumber :(parseInt(pageNumber)-1)}`}>
                <div onClick={leftClick} 
                className={`w-[80px] aspect-square bg-white border-2 ${pageNumber == 1 ? 'border-gray-400 text-gray-400 cursor-default':'border-[#22E5DE] text-[#22E5DE] hover:text-[#1BB6B1] hover:border-[#1BB6B1]'} absolute left-[calc(50%-25px)] top-[calc(50%-25px)]  opacity-0 group-hover:opacity-100  duration-1000 flex justify-center items-center rounded-[50%] cursor-pointer`}
                >
        ...
        <div className='group absolute top-[0]  right-[0px] z-10 w-[80px] h-[100%] bg-transparent text-[#22E5DE]' >
            {tektek && 
            <Link href={`/manga/${mangaName}/${episodeNumber}/${ pageNumber == fileList[fileList.length-1].sayiDegeri ? pageNumber :(parseInt(pageNumber)+1)}`}>
                <div onClick={rightClick} 
                className={`w-[80px] aspect-square bg-white border-2 ${pageNumber == fileList[fileList.length-1].sayiDegeri ? 'border-gray-400 text-gray-400 cursor-default':'border-[#22E5DE] text-[#22E5DE] hover:text-[#1BB6B1] hover:border-[#1BB6B1]'} absolute right-[calc(50%-25px)] top-[calc(50%-25px)] opacity-0 group-hover:opacity-100 duration-1000 flex justify-center items-center rounded-[50%] cursor-pointer`}>
