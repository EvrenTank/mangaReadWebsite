...
const SelectorDiv = ({iconOption,pageNumber,setFirstpagenumber,firstpagenumber,onClickEvent,setIndex,neededPageNumber }:any) => {
    return (
        ...
        <div 
            ...
            ${neededPageNumber<pageNumber ? 'bg-gray-400' : 'hover:bg-[#685eff] text-[#8992b7] hover:text-white group cursor-pointer' } `}
         onClick={
         ...
         >
            ...
            {(iconOption == undefined) && <h1 className={`${pageNumber === firstpagenumber ? 'text-white' : neededPageNumber<pageNumber ? 'text-gray-200':'text-[#8992b7]'} group-hover:text-white `}>{pageNumber}</h1> }
